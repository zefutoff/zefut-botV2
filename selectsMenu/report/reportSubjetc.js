const { TextInputBuilder, Client, EmbedBuilder, ModalBuilder } = require('discord.js');
...
module.exports = {
    ...
    async execute(client, interaction) {
        const modal = new ModalBuilder().addComponents(
            new TextInputBuilder().setCustomId('test').setLabel('Explique nous la situation').setStyle('LONG').setMinLength(100).setRequired(true)
        );
...
clientModal.on('modalSubmit', modal => {
    ...
    if (modal.customId === 'sanction_report') {
        modal.guild.channels.cache
            .get(administrator)
            .send({ embeds: [report.setTitle(`${modal.member.user.username} conteste une sanction`).setDescription(modal.fields[0].value)] });
    ...
    if (modal.customId === 'bug_report') {
        modal.guild.channels.cache
            .get(administrator)
            .send({ embeds: [report.setTitle(`${modal.member.user.username} à trouver un bug`).setDescription(modal.fields[0].value)] });
        modal.reply({ embeds: [response.setDescription(`✅ ta demande à était transmise aux administrateurs !`)], ephemeral: true });
    }
    if (modal.customId === 'moderator_report') {
        modal.guild.channels.cache
            .get(administrator)
            .send({ embeds: [report.setTitle(`${modal.member.user.username} rencontre un problème avec un modérateur`).setDescription(modal.fields[0].value)] });
        modal.reply({ embeds: [response.setDescription(`✅ ta demande à était transmise aux administrateurs !`)], ephemeral: true });
    }
    if (modal.customId === 'administrator_report') {
        modal.guild.members.cache
            .get('386931324042084354')
            .send({ embeds: [report.setTitle(`${modal.member.user.username} rencontre un problème avec un administrateur`).setDescription(modal.fields[0].value)] });
